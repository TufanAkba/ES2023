...
if __name__ == "__main__":
    ...
    d_nom = np.loadtxt(folder + '/d_nom.csv')
    porosity = np.loadtxt(folder + '/porosity.csv')
    ...
    surr.add_input('d_nom',training_data=d_nom)
    surr.add_input('porosity',training_data=porosity)
    ...
    # dataset = pd.read_csv('train100.csv')
    # print(dataset.columns) # get column names
    # inputs=dataset[['rpc','Tfi','m_dot','ins','L','d_nom','porosity']]
    # outputs=dataset[['Tfo','vol','T_o']]
    # outputs=dataset[['Tfo','T_o']]
    ...
    for i in range(Tfo.size):
        ...
        p.set_val('L',L[i])
        p.set_val('ins',ins[i])
        p.set_val('d_nom',d_nom[i])
        p.set_val('porosity',porosity[i])
