...
def sankey(prob, pt):
    ...
    m = prob.get_val(pt+'.balance.W', units='kg/s')
    ...
    P_rec_in = prob.get_val(pt+'.rec.Q_Solar_In', units='kW')
    ...
    P_MG_in = prob.get_val(pt+'.shaft.pwr_net', units='kW')
    P_MG_out = prob.get_val(pt + '.balance.pwr_target', units='kW')
