...
class surr(om.MetaModelUnStructuredComp):
    ...
    def setup(self):
        ...
        self.add_input('d_nom',training_data=np.loadtxt(test_folder + '/d_nom.csv'), units='m')
        self.add_input('porosity',training_data=np.loadtxt(test_folder + '/porosity.csv'))
...
class surrOpt(om.ExplicitComponent):
    ...
    def initialize(self):
        ...
        self.options.declare('d_nom', types=float, default=0.00254, desc='Nominal pore size in m')
        self.options.declare('porosity',types=float, default=0.81, desc='RPC porosity values')
    ...
    def setup(self):     
        ...
        self.add_output('porosity', desc='Porosity RPC')
        self.add_output('d_nom',desc='Nominal pore size', units='m')
        ...
        prob.model.set_input_defaults('d_nom',self.options['d_nom'], units='m')
        prob.model.set_input_defaults('porosity', self.options['porosity'])
        ...
        prob.model.add_design_var('d_nom',lower=0.0005, upper=0.01)
        prob.model.add_design_var('porosity',lower=0.20, upper=0.95)
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['d_nom'] = prob['d_nom']
        outputs['porosity'] = prob['porosity']
...
if __name__ == "__main__":
    ...
    # p.model.add_subsystem('surr', surr(test_folder=test_folder))
    # p.setup()
    # p.run_model()
